...
<body>
	...
	String endmonth = "0";
	String endday = "0";
	Vector<AdminForm_result> adminVresult = null;
	Vector<CompanySidStruct> vcompanysid = new Vector<CompanySidStruct>();
	...
	<%
		...
		vcompanysid.clear();
		conn.openStatement();
		conn.fillCompanySid(vcompanysid);
		System.out.println("jsp company id size : " + vcompanysid.size());
		conn.closeStatement();
		...
		if (datestruct != null && type_checkboxs != null && sid_checkboxs != null){
			beginDate = datestruct.begindate;
			endDate = datestruct.enddate;
			session.setAttribute("beginDate", beginDate);
			session.setAttribute("endDate", endDate);
			System.out.println("datastruct is not null" + beginDate + endDate);
			adminVresult = (Vector<AdminForm_result>) session.getAttribute("admin-form-result-vector");
			if(adminVresult == null){
				conn.openStatement();
				adminVresult = new Vector<AdminForm_result>();
				conn.fillAdminFormResult(adminVresult, beginDate, endDate);
				session.setAttribute("admin-form-result-vector", adminVresult);
				conn.closeStatement();
			}
			else {
				
				conn.openStatement();
				System.out.println("admin v result : " + adminVresult.size() + beginDate + endDate);
				adminVresult.clear();
				conn.fillAdminFormResult(adminVresult, beginDate, endDate);
				session.setAttribute("admin-form-result-vector", adminVresult);
				conn.closeStatement();
		...
		if (type_checkboxs != null && sid_checkboxs != null)
		{
			...
			{
				for(AdminForm_result af : adminVresult){
					for (form_result f : af.vresult)
					{
						if(s.equals(f.sid))
						{
	...
	<div id="wrapper">
        		...
        		<div class="col-lg-12">
        			<div class="panel panel-default">
        				...
        				<div class="panel-body">
	        				<form action="adminForm_generate.jsp" onsubmit="return disable_button();">
	        					...
	        					<div class="form-group">
	        						...
	        						<%
	        						for (CompanySidStruct css : vcompanysid){
	        							%>
	        							<h2><%=css.company %></h2>
	        							<%
	        							for(String sid : css.vsid)
		        						{
		        							%>
		        							<div class="checkbox-inline">
		        								<label>
		        									<input type="checkbox" value="<%=sid %>" name="sid-checkboxs" class="<%="sid-checkboxs-class-" + String.valueOf(vcompanysid.indexOf(css)) %>">
		        									<%=sid %>
		        								</label>
		        							</div>
		        							<% 
	        							...
	        							<div class="checkbox">
	        							<label>
											<input type="checkbox" value="all-check" onclick="<%="selectAll" + String.valueOf(vcompanysid.indexOf(css)) + "()" %>" id="<%=String.valueOf(vcompanysid.indexOf(css))  + "-is-allcheck" %>">
											<%=css.company %>机床全选 			
	        							</label>
        ...
        if (is_get_file)
        {
        	...
        	for (AdminForm_result af : adminVresult){
            	...
            	String zippath = fileDir + af.company + "all.zip";
            	...
            	%>
            	<div class="row huge">
        			<a href="<%="./outputsheet/" + af.company + "all.zip" %>" download = "<%=af.company + "all.zip" %>"><%=af.company %>全部下载（压缩文件.zip）</a>
        		</div>
            	<%
        	}
        }
    ...
    for (CompanySidStruct css : vcompanysid){
    	...
    	function <%="selectAll" + String.valueOf(vcompanysid.indexOf(css)) %>() {
    		var allchecked = document.getElementById("<%=String.valueOf(vcompanysid.indexOf(css))  + "-is-allcheck" %>");
			var all = document.getElementsByClassName("sid-checkboxs-class-" + <%="\"" + String.valueOf(vcompanysid.indexOf(css)) + "\"" %>);
			//console.log(all)

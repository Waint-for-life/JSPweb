...
%>
	 <div id="wrapper">
        ...
        <div id="page-wrapper">
                        ...
                        <div class="panel-heading">
                            <div class="row">
                                ...
                                <div class="col-xs-9 text-right">
                                    ...
                                    <div class="huge">错误警报</div>
                                </div>
                            </div>
                        </div>
                        <a href="#">
                            <div class="panel-footer">
                                <span class="pull-left">点击此处查看警报详情！</span>
            ...
            <%
            for (int i = 0; i < tab_index -1; i++) {	//标签页（panel）总数，整组
            	%>
            	<div class="row">
            		<div class="col-lg-12">
            			<div class="panel panel-default">
            				<div class="panel-heading">
            					<%="设备列表（每" + numPerTab + "台一组）" %>
            				</div>
            				<div class="panel-body">
            					<ul class="nav nav-pills">
            						<li class="active" id=<%="\"" + "tableindex-" + i + "0" + "\"" %>>
            							<a href=<%="\"#" + resultvector.get(i * numPerTab ).tabid + "\"" %> data-toggle="tab" id=<%="\"" + resultvector.get(i * numPerTab ).a_id + "\"" %>>
            								<%= "设备号：" + resultvector.get(i * numPerTab ).sid %>
            							</a>
            						</li>
            						<%
            						for (int j = 1; j < numPerTab; j++){
            							%>
            							<li id=<%="\"" + "tableindex-" + i + j + "\"" %>>
            								<a href=<%="\"#" + resultvector.get(i * numPerTab + j).tabid + "\"" %> data-toggle="tab" id=<%="\"" + resultvector.get(i * numPerTab + j).a_id + "\"" %>>
            									<%= "设备号：" + resultvector.get(i * numPerTab + j).sid %>
            								</a>
            							</li>
            					...
            					<div class="tab-content">
            						<%
            						for (int j = 0; j < numPerTab; j ++)
            						{
            							result res = resultvector.get(i * numPerTab + j);
            							%>
							            			...
							            			<div class="panel panel-default">
							            				...
							            				<div class="panel-body">
							            					<div class="table-responsive">
							                                    ...
							                                    <tbody>
							                                    <%for (int k = 0 ; k < res.alarm_no_v.size(); k ++){
							                                    	%>
							                                    	
							                                    		<tr>
								                                    		<td><%=res.login_time_v.get(k) %></td>
								                                    		<td><%=res.on_time_v.get(k) %></td>
								                                    		<td><%=res.run_no_v.get(k) %></td>
								                                    		<td><%=res.alarm_no_v.get(k) %></td>
								                                    		<td><%=res.span_no_v.get(k) %></td>
							                                    		<tr>
							            						...
							            						<p>
							            							<a href="#">
						            								<button type="button" class="btn btn-outline btn-danger btn-lg">告警总数详情</button>
						            							</a>
						            							<a href="#">
						            								<button type="button" class="btn btn-outline btn-warning btn-lg">间隔节点详情</button>
						            							</a>
						            							<a href="#">
						            								<button type="button" class="btn btn-outline btn-success btn-lg">信息总数详情</button>
						            							</a>
							            						</p>
							            					</div>
							            				</div>
      		...
      		<div class="row">
           		<div class="col-lg-12">
           			<div class="panel panel-default">
           				...
           				<div class="panel-body">
           					<ul class="nav nav-pills">
           						<li class="active" id=<%="\"" + "tableindex-" + tab_index + "0" + "\"" %>>
           							<%
           							if (tab_index > 0){
           								%>
           								<a href=<%="\"#" + resultvector.get((tab_index -1 ) * numPerTab ).tabid + "\"" %> data-toggle="tab" id=<%="\"" + resultvector.get((tab_index -1 ) * numPerTab ).a_id + "\"" %>>
           								<%= "设备号：" + resultvector.get((tab_index -1 ) * numPerTab ).sid %>
           								</a>
           								<%
           							}
           							%>
           						...
           						<%
           						for (int j = 1; j < remain_index; j++){
           							result res =  resultvector.get((tab_index -1 ) * numPerTab + j);
           							%>
           							<li id=<%="\"" + "tableindex-" + tab_index + j + "\"" %>>
           								<a href=<%="\"#" + res.tabid + "\"" %> data-toggle="tab" id=<%="\"" + resultvector.get((tab_index -1 )*numPerTab + j ).a_id + "\"" %>>
           									<%= "设备号：" + res.sid %>
           								</a>
           							</li>
           					...
           					<div class="tab-content">
           						<%
           						for (int j = 0; j < remain_index; j ++)
           						{
           							result res = resultvector.get((tab_index - 1 ) * numPerTab + j);
           							%>
           							
           							<div class="tab-pane fade in active" id=<%="\"" + res.tabid + "\"" %>>
           								<div class="row">
						            		<div class="col-lg-6">
						            			<div class="panel panel-default">
						            				<div class="panel-heading">
